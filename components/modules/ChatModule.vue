...
const sendMessage = () => {
  const action = props.webrtc.getAction('chat');

  action.setAttributes({
    message: text.value,
    sender: props.webrtc.peerJsId,
    private: (selectedUser.value),
  });

  if(selectedUser.value) {
    action.setUsers(selectedUser.value);
  }

  action.request();

  insertMessage('You', text.value, (selectedUser.value));
  text.value = null;
};

const setActionEventListener = () => {
  window.addEventListener('onChatMessageReceived', receivedMessageAction);
...
const handleEmojiesBox = (event) => {
  if (!(event.target.classList.contains('emojies-toggler') ||
    emojiesBox.value === event.target ||
    emojiesBox.value.contains(event.target))) {
    emojiesBox.value.classList.remove('show')
  }
};

onUnmounted(() => {
  window.removeEventListener('onChatMessageReceived', receivedMessageAction);
